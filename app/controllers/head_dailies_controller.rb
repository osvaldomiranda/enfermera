class HeadDailiesController < ApplicationController
  ...
  def index
    ...
    @head_dailies = HeadDaily.with_tipo(@tipo).with_numero(@numero).order(numero: :desc).page(params[:page]).per_page(20) 
  ...
  private
    ...
    def head_daily_params
      params.require(:head_daily).permit(:numero, :user_id, :tipo, :paguesea, :por, :mediopago, :banco, :recibidode, :numcheque, :documento, :fecha_contable)
    end
end
